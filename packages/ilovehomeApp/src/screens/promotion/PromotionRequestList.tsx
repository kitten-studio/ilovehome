...
type PromotionRequestListType = StackScreenProps<AppStackParamList, "PromotionRequestList"> & {
	userId: string
}

const PromotionRequestList: FC<PromotionRequestListType> = ({ navigation, route }) => {
	const fetchDataLoading = {
		queryKey: ["product/promotion/request/list"],
		url: "product/promotion/request/list",
		data: {
			userId: route.params?.userId,
		},
	}
	const { data, isError, isLoading } = useDataLoadingWith<ProductListResponse[]>(fetchDataLoading)
	useEffect(() => {
			...
			<TouchableOpacity
				...
				<View
					...
					<View
						...
						{/* <Image
							source={getImageThumbnail(item.image_thumbnail)}
							style={{ width: 120, height: 88, borderRadius: 5 }}
						/> */}
						<AutoHeightImage
					...
					<View
						...
						<Text style={{ fontStyle: "normal", color: "#0033B7", fontWeight: "700" }}>{`${getPrice(
							item.price
						)} P`}</Text>
						<Text
							style={{
	...
	return (
		<>
			<Topbar title={"요청중인 답례품 리스트"} isLeft />
